...
Parser.prototype.parse = function(args, options) {
  ...
  function parseUnflagged() {
      ...
      if(param == null) {
        error('unexpectedValue', { message: 'No value expected: ' + value, parser: parser, index: i, value: value });
      }
  ...
  function parseFlaggedParam(arg, mayConsumeNext) {
    ...
    if(param.isSwitch) {
      ...
      if(!param.optionalValue && value != null) {
        ...
        error('unexpectedValue', { message: 'No value expected: ' + flag, parser: parser, param: param, index: i });
